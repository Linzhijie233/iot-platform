    ...
    },
    {
      ...
      render: (_: number, record: ConfigRecord) => (
        <InputNumber
          ...
          min={0}
          style={{ width: 140 }}
          value={record.remainTraffic}
          onChange={(value) =>
            setRows((list) =>
              list.map((item) =>
                item.key === record.key
                  ? { ...item, remainTraffic: Number(value || 0) }
                  : item,
              ),
            )
          }
    ...
    },
    {
      ...
      render: (_: string, record: ConfigRecord) => (
        <Space>
          ...
          <Select
            options={[
              { label: '100GB流量池加油包', value: '100GB流量池加油包' },
              { label: '500GB流量池加油包', value: '500GB流量池加油包' },
              { label: '1000GB流量池加油包', value: '1000GB流量池加油包' },
            ]}
            style={{ width: 220 }}
    ...
    >
      ...
      <div className={styles.noticeBox}>
        ...
        <ol style={{ margin: '12px 0 0 18px', padding: 0 }}>
          <li>
            开启本功能后，流量池使用量达到所设置的条件时，会自动续费所配置的流量池叠加包，以保证所有流量池卡正常使用，不会停机；
          </li>
