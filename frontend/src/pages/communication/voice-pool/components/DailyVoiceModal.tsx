...
const dailyData = Array.from({ length: 10 }).map((_, index) => ({
  ...
  poolName: 'C000003-CMPS100M-01',
  usage: 0,
  fee: '--',
}));

const DailyVoiceModal: React.FC<Props> = ({ open, poolName, onClose }) => {
    ...
    >
      ...
      <Table
        columns={[
          ...
          { title: '语音池', dataIndex: 'poolName' },
          { title: '通话时长(分钟)', dataIndex: 'usage' },
...
export default DailyVoiceModal;